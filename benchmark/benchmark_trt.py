...
class ModelBenchmark:
    ...
    def run_comprehensive_benchmark(
        ...
        input_shapes: List[Tuple[int, int]] = [(640, 640)],
        backends: List[str] = ["cpu", "cuda", "tensorrt"],
        precisions: Dict[str, List[str]] = None
    ):
                    ...
                    for input_shape in input_shapes:
                        try:
                            ...
                            result = self.benchmark_session(
                                session, input_data, backend, precision,
    ...
    def print_results_table(self):
        ...
        print("="*100)
        print(f"{'Backend':<15} {'Precision':<10} {'Batch':<8} {'Shape':<12} "
    ...
    def calculate_speedups(self):
        ...
        for batch_size in set(r.batch_size for r in self.results):
            for input_shape in set(r.input_shape for r in self.results):
                # Find baseline
                baseline = next(
                    (r for r in self.results
                     if r.backend == "cpu" and r.precision == "FP32"
                     and r.batch_size == batch_size and r.input_shape == input_shape),
                    None
                )
